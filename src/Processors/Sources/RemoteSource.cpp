...
ISource::Status RemoteSource::prepare()
{
    ...
    {
        LOG_FATAL(&Poco::Logger::get("Anime"), "RemoteSource::prepare will return finish");
        getPort().finish();
    ...
    if (is_async_state)
    {
        LOG_FATAL(&Poco::Logger::get("Anime"), "RemoteSource::prepare will return Async");
        return Status::Async;
    }
    ...
    {
        LOG_FATAL(&Poco::Logger::get("Anime"), "RemoteSource::prepare will finish query executor");
        query_executor->finish(&read_context);
        is_async_state = false;
    }

    LOG_FATAL(&Poco::Logger::get("Anime"), "RemoteSource::prepare will return status {}", statusToName(status));
    return status;
}

std::optional<Chunk> RemoteSource::tryGenerate()
{
    LOG_FATAL(&Poco::Logger::get("Anime"), "RemoteSource::tryGenerate");
